...
| <a name="input_billing_notification_emails"></a> [billing\_notification\_emails](#input\_billing\_notification\_emails) | List of email addresses to subscribe to budget sns topic | `list(string)` | `[]` | no |
...
| <a name="input_environment"></a> [environment](#input\_environment) | ID element. Usually used for region e.g. 'uw2', 'us-west-2', OR role 'prod', 'staging', 'dev', 'UAT' | `string` | `"debug"` | no |
